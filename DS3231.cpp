...
char *DS3231::getDOWStr(uint8_t format)
{
	...
	char *daysLong[]  = {"Poniedziałek", "Wtorek", "Środa", "Czwartek", "Piątek", "Sobota", "Niedziela"};
	char *daysShort[] = {"Pon", "Wto", "Śro", "Czw", "Pią", "Sob", "Nie"};
	Time t;
	t=getTime();
	if (format == FORMAT_SHORT)
...
char *DS3231::getMonthStr(uint8_t format)
{
	...
	char *monthLong[]  = {"Styczeń", "Luty", "Marzec", "Kwiecień", "Maj", "Czerwiec", "Lipiec", "Sierpień", "Wrzesień", "Październik", "Listopad", "Grudzień"};
	char *monthShort[] = {"Sty", "Lut", "Mar", "Kwi", "Maj", "Cze", "Lip", "Sie", "Wrz", "Paź", "Lis", "Gru"};
	Time t;
	t=getTime();
	if (format == FORMAT_SHORT)
